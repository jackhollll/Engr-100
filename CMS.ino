...
char appData;
String inData = "";
const uint8_t outPins[4] = {2, 3, 4, 5};  // Output pins
      ...
      case 2:
        {
          int sensorValue = analogRead(sensorAPins[port]);
          int outputValue = map(sensorValue, 0, 1023, 100, 0);
          Serial.println("Sensor Port " + String(port+1) + ":\tLight:\t" + String(outputValue) + "\n");
        }
        break;
      case 3:
        {
          int sensorValue = analogRead(sensorAPins[port]);
          int outputValue = map(sensorValue, 0, 1023, 100, 0);
          output += ("Sensor Port " + String(port+1) + ":\tCO2:\t" + String(outputValue) + "\n");
        }
        break;
      case NO_SENSOR:
        {
